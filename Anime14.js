document.addEventListener("DOMContentLoaded", async function () {
    ...
    function updateAnimePage(anime) {
        ...
        // Real-time Airing Countdown
        if (anime.nextAiringEpisode) {
            let time = anime.nextAiringEpisode.timeUntilAiring;
            const paddedEp = String(anime.nextAiringEpisode.episode).padStart(3, "0");

            function updateCountdown() {
                if (time <= 0) {
                    document.getElementById("anime-airing").textContent = `EP ${paddedEp} is airing now!`;
                ...
                let airingText = `EP ${paddedEp} airs in `;
                if (days > 0) airingText += `${days} days `;
                airingText += `${hours} hrs ${minutes} min ${seconds} sec`;

                document.getElementById("anime-airing").textContent = airingText;
                time--;
            }
